import { RoomState, UIElements } from './types.ts';
import { MessageDisplay } from './chat.ts';
import { EventEmitter, getRequiredElement, Logger } from './utils.ts';
...
export class UIManager extends EventEmitter<UIEvents> {
	...
	private getUIElements(): UIElements {
		return {
			...
			connectionStatus: getRequiredElement<HTMLDivElement>(
				'connectionStatus',
			),
		};
	}
	...
	addMessage( message: MessageDisplay ): void {
		...
		const isNearBottom =
			messages.scrollHeight - messages.scrollTop - messages.clientHeight <
				config.ui.scrollToBottomThreshold;
		if ( isNearBottom ) {
