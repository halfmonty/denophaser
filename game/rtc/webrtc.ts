import { ExtendedRTCPeerConnection } from './types.ts';
...
export class WebRTCManager extends EventEmitter<WebRTCEvents> {
	...
	): Promise<ExtendedRTCPeerConnection> {
		this.logger.log(
			`Creating connection to ${userId}, createDataChannel: ${createDataChannel}`,
		);
	...
	closeAllConnections(): void {
		...
		this.connections.forEach( ( _pc, userId ) => {
			this.closeConnection( userId );
		} );
	}

	private setupPeerConnectionHandlers(
		pc: ExtendedRTCPeerConnection,
		userId: string,
	): void {
		...
		pc.onconnectionstatechange = () => {
			this.logger.log(
				`Connection state with ${userId}: ${pc.connectionState}`,
			);
